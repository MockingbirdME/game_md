...
* **Flurry of blows(2)** - make a basic attack and choose to spend between one an three additional stamina, the attack gains a number of bonus dice equal to the number of extra stamina spent plus one; if the skill check succeeds resolve it as a single attack with no additional successes.
* **Lunge(2)** - make a normal melee skill check; double the character’s, not the weapon’s, reach for this attack. Lose any parry bonus from the weapon used until the beginning of your next turn.
* **Off-hand attack(1)** - make a basic attack with a readied weapon the character is wielding in their off hand. This attack has a penalty die added to the skill check and has a turn requirement of zero the first time the character uses it on their turn. The weapon looses its parry bonus until the beginning of the character's next turn.
...
* **Reckless attack(2)** - make a basic attack and choose a number between one and three, add that many bonus dice to the attack's skill check and the attacking character loses twice that many defense.
