...
* **Lunge(2)** - make a basic attack treating the character’s, not the weapon’s, reach as double its standard value for this attack. This attack's skill check has one penalty die added to it and the character loses any parry bonus from the weapon used until the beginning of their next turn unless the weapon or attack used has the thrust keyword.
...
* **Power attack(2)** - make a basic attack with two penalty dice added to the skill check, zero penalty dice if this is the first attack after the character performed a dash action this turn; the character treats his body stat as if it were 1 greater for the purposes of calculating damage on this attack, additionally each additional success spent to increase the attacks damage increases it by an additional one.
